...
class RsControllerTest {
    ...
    public void should_add_rsEvent_when_addRsEvent_given_new_rsEvent() throws Exception {
        ...
        String newRsEventStr = objectMapper.writeValueAsString(new RsEvent("猪肉涨价了", "经济", new User("wjl", "male", 18, "wangjianlin@demo.com", "11122223333")));
    ...
    public void should_add_new_user_when_addRsEvent_given_new_rsEvent_and_new_user() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        String newRsEventStr = objectMapper.writeValueAsString(new RsEvent("猪肉涨价了", "经济", new User("wjll", "male", 18, "wangjianlin@demo.com", "11122223333")));
        mockMvc.perform(post("/rs/event").content(newRsEventStr).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    ...
    public void should_update_when_updateRsEvent_given() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        String newRsEventStr = objectMapper.writeValueAsString(new RsEvent("事件更改了", "无标签", new User("wjl", "male", 18, "wangjianlin@demo.com", "11122223333")));
